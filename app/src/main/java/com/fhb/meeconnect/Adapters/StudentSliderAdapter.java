...
public class StudentSliderAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(@NonNull final ViewGroup container, final int position) {
        ...
        TextView PhoneNumber = view.findViewById(R.id.detail_number);
        ...
        if(posts.get(position).getMessengerID().trim().equals("NA")){
            ...
            messenger.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    ...
                    String MESSENGER = posts.get(position).getMessengerID().trim().toLowerCase();

                    ClipboardManager clipboard = (ClipboardManager) context.getSystemService(Context.CLIPBOARD_SERVICE);
                    ClipData clip = ClipData.newPlainText("clipboard data", MESSENGER);
                    clipboard.setPrimaryClip(clip);

                    Vibrator vibrator = (Vibrator) context.getSystemService(context.VIBRATOR_SERVICE);
                    vibrator.vibrate(400);

                    Toasty.success(context, "Link copied to clipboard!", Toast.LENGTH_SHORT, true).show();
                    return true;
        ...
        phone.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                String PHONE =posts.get(position).getPhone().trim();

                ClipboardManager clipboard = (ClipboardManager) context.getSystemService(Context.CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("clipboard data",PHONE);
                clipboard.setPrimaryClip(clip);

                Vibrator vibrator = (Vibrator) context.getSystemService(context.VIBRATOR_SERVICE);
                vibrator.vibrate(400);

                Toasty.success(context, "Number copied to clipboard!", Toast.LENGTH_SHORT, true).show();
                return true;
            }
        });

        name.setText(posts.get(position).getName().trim());
        PhoneNumber.setText(posts.get(position).getPhone().trim());
        nickname.setText(posts.get(position).getNickname().trim());
        regNo.setText(posts.get(position).getRegistrationNo().trim());
